...
/** 
 * \brief Lifts a point from the image plane to its projective ray 将一个点从像平面提升到其投影射线
 *
 * \param p image coordinates 图像坐标
 * \param P coordinates of the projective ray 投射射线的坐标
...
{
    double tol = 1e-10;
    double p_u_norm = p_u.norm();//求向量模长
    ...
    {
        // Get eigenvalues of companion matrix corresponding to polynomial. 获取与多项式相对应的伴随矩阵的特征值。
        // Eigenvalues correspond to roots of polynomial. 特征值对应于多项式的根。
