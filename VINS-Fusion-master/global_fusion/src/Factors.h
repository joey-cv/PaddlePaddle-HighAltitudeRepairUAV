...
struct TError
{
	...
	/**
	 *TError（x,y,z,accuracy），最后一项是定位精度，可以由GPS系统提供。
	 残差除了直接观测值与真值相减以外，还除了这个accuracy作为分母。
	 意味着精度越高，accuracy越小，对结果的影响就越大。 
	 **/
...
struct RelativeRTError
{
	...
	/**
	 * RelativeRTError（dx,dy,dz,dqw,dqx,dqy,dqz，t_var,q_var），
	 * 最后两项是位移以及旋转之间的权重分配比例，并且为了使得与TError对应。
	 * 在程序中，应该是根据经验把最后两项设置成一个常值，分别对应0.1以及0.01。
	 * 残差的得到就根据实际值与观测值之间的偏差直接得出。
