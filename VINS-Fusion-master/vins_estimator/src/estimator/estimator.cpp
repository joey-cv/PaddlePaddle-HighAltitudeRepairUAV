...
void Estimator::setParameter()
{
    ...
    featureTracker.readIntrinsicParameter(CAM_NAMES);//读取相机内参读取每个相机对应内参，单目时NUM_OF_CAM=1
...
void Estimator::inputIMU(double t, const Vector3d &linearAcceleration, const Vector3d &angularVelocity)
{
    ...
    {
        mPropagate.lock();
        fastPredictIMU(t, linearAcceleration, angularVelocity);//计算最新pvq
        pubLatestOdometry(latest_P, latest_Q, latest_V, t);//发布最新消息
        mPropagate.unlock();
    }
}
        ...
        if(!featureBuf.empty())
        {
            ...
            if(USE_IMU)
            {
                if(!initFirstPoseFlag)
                    initFirstIMUPose(accVector);//求一个姿态角,然后把航向角设为0
...
//其实很简单,就是求一个姿态角,然后把航向角设为0，初始化当前一段时间内的加速度计的数据
...
bool Estimator::visualInitialAlign()
{
    ...
    VectorXd x;
    //solve scale
    bool result = VisualIMUAlignment(all_image_frame, Bgs, g, x);// 视觉IMu的对其更新得到新的陀螺仪漂移Bgs
    if(!result)
    {
...
void Estimator::optimization()
{
    ...
    double2vector();//把ceres求解出来的结果附加到滑窗内的变量中去
