        ...
        for (auto &it_per_id : feature)
        {
            if (it_per_id.estimated_depth > 0)
            {
                ...
                if((int)it_per_id.feature_per_frame.size() >= index + 1)
                {
                    ...
                    Vector3d ptsInWorld = Rs[it_per_id.start_frame] * ptsInCam + Ps[it_per_id.start_frame];//得到世界坐标系下的三维坐标
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        if(STEREO && it_per_id.feature_per_frame[0].is_stereo)
        {
            ...
            Eigen::Vector3d t1 = Ps[imu_i] + Rs[imu_i] * tic[1];//利用imu的位姿
            ...
            localPoint = leftPose.leftCols<3>() * point3d + leftPose.rightCols<1>();//求左相机坐标
