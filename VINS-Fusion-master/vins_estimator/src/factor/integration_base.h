...
class IntegrationBase
{
  public:
    ...
    //??????Jacobian?Covariance
    // _acc_0??????? _acc_1??????? delta_p?????? result_delta_p????????? update_jacobian????????????????IMU??????????????sdf
    ...
    double dt;
    Eigen::Vector3d acc_0, gyr_0;//初始预积分的值
    Eigen::Vector3d acc_1, gyr_1;

    const Eigen::Vector3d linearized_acc, linearized_gyr;//后面更新时每一帧的acc和gyr的值
    Eigen::Vector3d linearized_ba, linearized_bg;//加速度的偏置以及陀螺仪的偏置,初始化的时候都为[0;0;0][0;0;0]

    Eigen::Matrix<double, 15, 15> jacobian, covariance;// 初始化为15×1515×15维的单位矩阵，初始化为15×1515×15维的零矩阵
    Eigen::Matrix<double, 15, 15> step_jacobian;
    Eigen::Matrix<double, 15, 18> step_V;
    Eigen::Matrix<double, 18, 18> noise;//初始化为18×1818×18维的噪声矩阵, 噪声项的对角协方差矩阵,imu标定拿到的参数

    double sum_dt;//初始化为0
    Eigen::Vector3d delta_p;//初始化为3×13×1维的单位矩阵
    Eigen::Quaterniond delta_q;//初始化为单位四元数
    Eigen::Vector3d delta_v;//初始化为零矩阵
