...
具体的，从文件中获取图像和GPS数据当前的时间戳信息。以第一帧图像和第一个GPS时间早的作为基准时间，
之后，左右双目的图像通过estimator.inputImage（）读入里程计中，里程计Estimator类内部会定时发送VIO计算的结果。
同时main函数中会在图片读入里程计的时刻发布同一时刻的GPS信息。（每一帧图片都有对应一条GPS信息，时间戳设置为一致的）
...
int main(int argc, char** argv)
{
	...
	{	
		if(ros::ok())
		{
			...
			std::fclose(GPSFile);
			//将gps数据封装进ros 的sensor_msgs::NavSatFix 数据类型，然后发布
