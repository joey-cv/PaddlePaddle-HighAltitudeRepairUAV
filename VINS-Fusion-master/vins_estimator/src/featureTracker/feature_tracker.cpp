...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    */
    ...
    cur_un_pts = undistortedPts(cur_pts, m_camera[0]);//将像素座标系下的座标，转换为归一化相机座标系下的座标 即un_pts为归一化相机座标系下的座标。
    pts_velocity = ptsVelocity(ids, cur_un_pts, cur_un_pts_map, prev_un_pts_map);//计算当前帧相对于前一帧 特征点沿x,y方向的像素移动速度
...
{
    vector<cv::Point2f> un_pts;//un_pts为归一化相机座标系下的座标
    for (unsigned int i = 0; i < pts.size(); i++)
    {
        Eigen::Vector2d a(pts[i].x, pts[i].y);//图像坐标
        Eigen::Vector3d b;//投影射线
        cam->liftProjective(a, b);//这个函数是对鱼眼相机模型的标定及去畸变过程
                                            ...
                                            map<int, cv::Point2f> &cur_id_pts, map<int, cv::Point2f> &prev_id_pts)
{
    ...
    // caculate points velocity 计算点的速度
                               ...
                               map<int, cv::Point2f> &prevLeftPtsMap)
{
    ...
    if (!imRight.empty() && stereo_cam)
        cv::hconcat(imLeft, imRight, imTrack);//拼接图
    else
