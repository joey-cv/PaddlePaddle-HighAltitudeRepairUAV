...
//定义了全局姿态类,整个程序都是维护这个main函数里面定义的全局变量PoseGraph posegraph来进行的
class PoseGraph
{
public:
	PoseGraph();
	...
	void addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop);//外部调用类pose_graph的第一个函数。即通过findConnection（）函数判断是否有检测到回环，检测到后满足一定条件就把回环帧的序号添加optimize_buf内部。
	void loadKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop);//只要在LOAD_PREVIOUS_POSE_GRAPH有，即我们有原来的地图信息，加载的时候，才会发生。
...
private:
	...
	void optimize4DoF();//全局优化一个4自由度的过程
	void optimize6DoF();
	void updatePath();
