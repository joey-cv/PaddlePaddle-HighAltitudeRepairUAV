...
//外部调用类pose_graph的第一个函数。即通过findConnection（）函数判断是否有检测到回环，检测到后满足一定条件就把回环帧的序号添加optimize_buf内部。
/**
...
//只要在LOAD_PREVIOUS_POSE_GRAPH有，即我们有原来的地图信息，加载的时候，才会发生。
 ...
 * 
 * 
 * 若detectLoop()能够找到相似的一帧，此时，通过findConnection() 判断新旧两帧关联，匹配点大于25对，相对的偏航角位移小于30度并且相对位移少于20m，则返回true,其余情况均返回false。
 * 在true的条件下，更新参考系转化的变量。并且把当前帧的序号加入到optimize_buf中去。
...
/**全局优化一个4自由度的过程,该过程由于IMU的存在，使得位姿6自由度的问题一下子降了两维。
 * 并且线程每结束一次优化的过程，线程就会睡眠2秒钟，再继续。
 * optimize4DoF优化的前提条件：optimize_buf不等于空。
 * 一个是addKeyFrame()函数，一个是loadKeyFrame（）函数。
