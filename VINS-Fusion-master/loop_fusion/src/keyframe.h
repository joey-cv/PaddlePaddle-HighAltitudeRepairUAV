...
public:
	...
	vector<BRIEF::bitset> brief_descriptors;//构造出一个向量容器，用来存放所有特征点的描述子,只有当两个关键帧之间的位移量(T-last_t)>阈值SKIP_DIS时候，才会构造新的关键帧。但是程序默认的SKIP_DIS为0。就是除非T和last_t完全相等，否则就会进入构造函数。
